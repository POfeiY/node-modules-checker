...
import { getPackageManager } from './agent-entry'
import { listPackagesDependenciesRaw } from './agent-entry/list'
...
export async function listPackageDependencies(options: ListPackageDependenciesOptions): Promise<ListPackageDependenciesResult> {
  ...
  const result = await listPackagesDependenciesRaw(packageManager, options) as ListPackageDependenciesResult
